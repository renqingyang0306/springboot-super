...
public class RedisService {
    ...
    public Long delete(int dbIndex, String... keys) {
        RedisTemplate.LOCAL_DB_INDEX.set(dbIndex);
        return redisTemplate.execute(new RedisCallback<Long>() {
            @Override
            public Long doInRedis(RedisConnection connection) throws DataAccessException {
                if (keys.length == 1) {
                    return connection.del(((RedisSerializer<String>) redisTemplate.getKeySerializer()).serialize(keys[0]));
                } else {
                    byte[][] bytes = new byte[keys.length][];
                    for (int i = 0; i < keys.length; i++) {
                        bytes[i] = ((RedisSerializer<String>) redisTemplate.getKeySerializer()).serialize(keys[0]);
                    }
     ...
     * @param message
     */
    public void sendMassage(int dbIndex, String channel, Object message){
        RedisTemplate.LOCAL_DB_INDEX.set(dbIndex);
    ...
    public  Map<String, String> multiget(int dbIndex, Map<String, String> map) {
        RedisTemplate.LOCAL_DB_INDEX.set(dbIndex);
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (Map.Entry<String, String> entry : map.entrySet()) {
                    try {
                        ...
                        logger.error("RedisService.multiget error", e);
                    }
                }
                return null;
            }
    ...
    public <T> Map<String, T> multiget(int dbIndex, Map<String, String> map, Class<T> clazz) {
        RedisTemplate.LOCAL_DB_INDEX.set(dbIndex);
        ...
        if (resultList != null && resultList.size() == map.size()) {
            ...
            for (Map.Entry<String, String> entry : map.entrySet()) {
                ...
                if (result != null && !StringUtils.isEmpty(result.toString())) {
                    try {
                        resultMap.put(entry.getKey(), mapper.readValue(result.toString(), clazz));
                    } catch (Exception e) {
                        logger.error("RedisService.multiget error", e);
                    }
                }
     ...
     * 如:批量获取多个队列的queueMemberList
     */
    public <T> Map<String, List<T>> multigetList(int dbIndex, Map<String, String> map, Class<T> clazz) {
        RedisTemplate.LOCAL_DB_INDEX.set(dbIndex);
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (Map.Entry<String, String> entry : map.entrySet()) {
                    try {
                        ...
                        logger.error("RedisService.multigetList error", e);
                    }
                }
                return null;
            }
            ...
            for (Map.Entry<String, String> entry : map.entrySet()) {
                ...
                if (result != null && !StringUtils.isEmpty(result.toString())) {
                    try {
                        ...
                        List<T> list = mapper.readValue(result.toString(), valueType);
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (Map.Entry<String, String> entry : map.entrySet()) {
                    try {
                        ...
                        logger.error("RedisService.multihgetall error", e);
                    }
                }
                return null;
            }
    ...
    public <T> List<T> hgetList(int dbIndex, String key, Class<T> clazz) {
        ...
        try {
            ...
            logger.error("RedisService.hgetList error", e);
        }
        return list;
    }
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (Map.Entry<String, String> entry : map.entrySet()) {
                    try {
                        ...
                        logger.error("RedisService.multihgetList error", e);
                    }
                }
                return null;
            }
    ...
    public <T> Boolean multihset(int dbIndex, String key, Map<String, T> map) {
        boolean result = true;
        RedisTemplate.LOCAL_DB_INDEX.set(dbIndex);
            ...
            for (Map.Entry<String, T> entry : map.entrySet()) {
                try {
                    if (entry.getValue() != null && entry.getValue() instanceof String) {
                        ...
                        valueMap.put(entry.getKey(), mapper.writeValueAsString(entry.getValue()));
                    }
                } catch (JsonProcessingException e) {
                    logger.error("RedisService.set error, key=" + key, e);
                    result = false;
                }
            }
            redisTemplate.opsForHash().putAll(key, valueMap);
        } catch (Exception e) {
            logger.error("RedisService.hmset error, key=" + key, e);
            result = false;
        }
        return result;
    }
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (String value : keyMap.values()) {
                    try {
                        ...
                        logger.error("RedisService.multihlen error", e);
                    }
                }
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (String value : keyMap.values()) {
                    try {
                        ...
                        logger.error("RedisService.multihget error", e);
                    }
                }
     ...
     * @return
     */
    public Map<String, String> multihgetstr(int dbIndex, String key, Map<String, String> map) {
        RedisTemplate.LOCAL_DB_INDEX.set(dbIndex);
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (String value : valueMap.values()) {
                    try {
                        ...
                        logger.error("RedisService.multisadd error", e);
                    }
                }
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                ...
                for (T member : memberList) {
                    connection.zAdd(((RedisSerializer<String>) redisTemplate.getKeySerializer()).serialize(key), scoreList.get(i)
                            , ((RedisSerializer<String>) redisTemplate.getValueSerializer()).serialize(JSONObject.toJSONString(member)));
                    i++;
                }
                return null;
            }
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (Map.Entry<String, String> entry : map.entrySet()) {
                    try {
                        ...
                        logger.error("RedisService.multizrank error", e);
                    }
                }
                return null;
            }
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                for (Map.Entry<String, String> entry : map.entrySet()) {
                    try {
                        ...
                        logger.error("RedisService.multizrange error", e);
                    }
                }
                return null;
            }
